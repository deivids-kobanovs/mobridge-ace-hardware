...
export function SummaryCards({ summary }: { summary: DashboardSummary }) {
  const cards: { label: string; value: string | number; icon: ComponentType<{ className?: string }>; tone: string }[] = [
    ...
    { label: 'Overdue', value: summary.overdue, icon: AlertTriangle, tone: 'text-rose-700 bg-rose-100' },
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7">
      {cards.map((c) => (
        <div key={c.label} className="rounded-xl border border-ink-200 bg-card p-4 shadow-card">
